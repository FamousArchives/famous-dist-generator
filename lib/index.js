...
function writeRequireJS (version, destination, minify, callback) {
  ...
  fetchFamous(version, function(err, famous) {
    if (err) { return callback(err); }
    var requireJS = Object.keys(famous).reduce(function(memo, key) {
      memo += famous[key] + '\n';
      return memo;
    }, '');
    if (minify) {
      var result = UglifyJS.minify(requireJS, {fromString: true});
      requireJS = licenseHeader + result.code;
    }
    fs.writeFile(destination, requireJS, 'utf8', callback);
  });
}
